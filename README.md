...
Техническое ядро MVP и локальная production-сборка готовы. Развёртывание на Beget начато, но production smoke test ещё не пройден. Для Давида подготовлен полный КТП математики 4 класса на 2026/27 год: 5 разделов, 63 темы и по одному уроку с мини-тестом на каждую тему.
...
В программе Давида родитель может нажать «Добавить всю математику 4 класса (63 темы)». Операция повторяемая: второй вызов не создаёт копии разделов, уроков и тестов. Пилотный маршрут по дробям устанавливается отдельно и используется стартовой диагностикой.
...
Фактическое состояние этапов и известный блокер production-развёртывания фиксируются в `docs/IMPLEMENTATION_STATUS.md`; этот файл следует обновлять после каждого законченного шага.
