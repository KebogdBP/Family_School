...
try {
  ...
  if (dump.length < 1024 || !dump.includes(Buffer.from('CREATE TABLE'))) throw new Error('Резервная копия пуста или не содержит схему')
  writeFileSync(join(temporary, 'homeedu.sql'), dump, { mode: 0o600 })
  mysqlServer(`DROP DATABASE IF EXISTS ${restored}; CREATE DATABASE ${restored} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`)
  restoreDatabase(restored, dump)
  const after = databaseSnapshot(restored)
  if (after !== before) throw new Error(`Состояние восстановленной базы отличается:\nSOURCE\n${before}\nRESTORED\n${after}`)
