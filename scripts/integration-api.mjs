...
const nextMonday = new Date(); nextMonday.setHours(12,0,0,0); nextMonday.setDate(nextMonday.getDate() + ((8 - (nextMonday.getDay() || 7)) % 7 || 7))
...
assert(planBeforeDraft.items.length === 0 && planDraft.items.length === 5, 'Plan draft must not change the schedule before parent approval')
const editedDate = new Date(`${nextWeekStart}T12:00:00`); editedDate.setDate(editedDate.getDate()+4)
await request(`/students/${david.student.id}/plan-items`, { method: 'POST', body: JSON.stringify({ lessonId: planDraft.items[0].lessonId, scheduledDate: editedDate.toISOString().slice(0,10), isRequired: false, position: 0 }) })
await request(`/students/${david.student.id}/plan-items`, { method: 'POST', body: JSON.stringify({ lessonId: planDraft.items[1].lessonId, scheduledDate: planDraft.items[1].scheduledDate, isRequired: true, position: 1 }) })
