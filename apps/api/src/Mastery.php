...
final class Mastery
{
    public static function record(PDO $db,string $familyId,string $studentId,string $topicId,string $type,string $sourceId,bool $successful,?int $score,string $explanation): void
    {
        ...
        if($successful&&$isDue){$status='mastered';$due=null;$db->prepare('UPDATE review_schedule SET status=\'completed\',completed_at=NOW() WHERE student_id=:student_id AND topic_id=:topic_id')->execute(['student_id'=>$studentId,'topic_id'=>$topicId]);}
