...
final class Mastery
{
    public static function record(PDO $db,string $familyId,string $studentId,string $topicId,string $type,string $sourceId,bool $successful,?int $score,string $explanation): void
    {
        ...
        $settings=$db->prepare('SELECT COALESCE(sms.min_evidence_count,2) AS min_evidence_count,COALESCE(sms.min_successful_types,2) AS min_successful_types,COALESCE(sms.review_interval_days,3) AS review_interval_days FROM topics t JOIN sections se ON se.id=t.section_id JOIN curriculum_subjects cs ON cs.id=se.curriculum_subject_id JOIN curricula c ON c.id=cs.curriculum_id LEFT JOIN subject_mastery_settings sms ON sms.curriculum_subject_id=cs.id WHERE t.id=:topic_id AND c.student_id=:student_id AND c.family_id=:family_id LIMIT 1');$settings->execute(['topic_id'=>$topicId,'student_id'=>$studentId,'family_id'=>$familyId]);$rules=$settings->fetch()?:['min_evidence_count'=>2,'min_successful_types'=>2,'review_interval_days'=>3];
        $due=$old['next_review_at']??null;$isDue=$due!==null&&$due<=date('Y-m-d');
        if(($old['status']??null)==='mastered'&&$successful){$status='mastered';$due=null;}
        elseif($successful&&$isDue){$status='mastered';$due=null;$db->prepare('UPDATE review_schedule SET status=\'completed\',completed_at=NOW() WHERE student_id=:student_id AND topic_id=:topic_id')->execute(['student_id'=>$studentId,'topic_id'=>$topicId]);Achievements::award($db,$familyId,$studentId,'durable_mastery','Тема освоена','Ты успешно подтвердил знания после перерыва.',$topicId);}
        elseif((int)$totals['evidence_count']>=(int)$rules['min_evidence_count']&&(int)$totals['successful_types']>=(int)$rules['min_successful_types']){$status='needs_reinforcement';$due=$due??date('Y-m-d',strtotime('+'.(int)$rules['review_interval_days'].' days'));$reason='Набрано подтверждений: '.$totals['evidence_count'].'; успешных типов работы: '.$totals['successful_types'].'. Повторение через '.$rules['review_interval_days'].' дн.';$db->prepare('INSERT INTO review_schedule(id,family_id,student_id,topic_id,due_date,reason) VALUES(:id,:family_id,:student_id,:topic_id,:due_date,:reason) ON DUPLICATE KEY UPDATE due_date=VALUES(due_date),reason=VALUES(reason),status=\'pending\',completed_at=NULL')->execute(['id'=>Uuid::v4(),'family_id'=>$familyId,'student_id'=>$studentId,'topic_id'=>$topicId,'due_date'=>$due,'reason'=>$reason]);}
