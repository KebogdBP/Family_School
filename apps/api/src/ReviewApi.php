...
final class ReviewApi
{
    ...
    private static function load(PDO $db,string $familyId,string $studentId,string $scheduleId,bool $requireDue): array
    {
        ...
        $configured=$db->prepare('SELECT COUNT(*) FROM review_question_settings WHERE family_id=:family_id AND student_id=:student_id AND topic_id=:topic_id');$configured->execute(['family_id'=>$familyId,'student_id'=>$studentId,'topic_id'=>$task['topic_id']]);
        if((int)$configured->fetchColumn()>0){$q=$db->prepare('SELECT q.id,q.prompt,q.question_type,q.options_json,q.correct_answer_json,q.explanation FROM review_question_settings rqs JOIN quiz_questions q ON q.id=rqs.question_id JOIN activities a ON a.id=q.activity_id JOIN lessons l ON l.id=a.lesson_id WHERE rqs.family_id=:family_id AND rqs.student_id=:student_id AND rqs.topic_id=:topic_id AND a.deleted_at IS NULL AND l.deleted_at IS NULL ORDER BY rqs.position LIMIT 3');$q->execute(['family_id'=>$familyId,'student_id'=>$studentId,'topic_id'=>$task['topic_id']]);}
        else{$q=$db->prepare('SELECT q.id,q.prompt,q.question_type,q.options_json,q.correct_answer_json,q.explanation FROM activities a JOIN quiz_questions q ON q.activity_id=a.id JOIN lessons l ON l.id=a.lesson_id WHERE l.topic_id=:topic_id AND a.activity_type=\'quiz\' AND a.deleted_at IS NULL AND l.deleted_at IS NULL ORDER BY a.position,a.created_at LIMIT 3');$q->execute(['topic_id'=>$task['topic_id']]);}
        $questions=$q->fetchAll();if($questions===[])Http::error('review_unavailable','Для темы пока нет тестовых вопросов',409);return[$task,$questions];
    }
}
