...
final class QuizApi
{
    ...
    private static function list(PDO $db, string $familyId, string $lessonId): never
    {
        ...
        $statement = $db->prepare('SELECT a.id,a.title,a.instructions,q.id AS question_id,q.prompt,q.question_type,q.options_json,q.correct_answer_json,q.explanation FROM activities a JOIN quiz_questions q ON q.activity_id=a.id WHERE a.lesson_id=:lesson_id AND a.family_id=:family_id AND a.activity_type=\'quiz\' AND a.deleted_at IS NULL ORDER BY a.position,a.created_at');
    ...
    private static function create(PDO $db, string $familyId, string $actorId, string $lessonId): never
    {
        ...
        if ($title === '' || $prompt === '' || !in_array($type, ['single_choice','multiple_choice','number','short_text'], true)) Http::error('validation_error', 'Проверьте название, вопрос и тип ответа', 422);
        ...
        try {
            ...
            $db->prepare('INSERT INTO quiz_questions (id,family_id,activity_id,prompt,question_type,options_json,correct_answer_json,explanation) VALUES (:id,:family_id,:activity_id,:prompt,:type,:options,:answer,:explanation)')->execute(['id'=>$questionId,'family_id'=>$familyId,'activity_id'=>$activityId,'prompt'=>$prompt,'type'=>$type,'options'=>$options === [] ? null : json_encode($options, JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR),'answer'=>json_encode($answer, JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR),'explanation'=>$explanation===''?null:$explanation]);
    ...
    private static function attempt(PDO $db, string $familyId, string $studentId, string $quizId): never
    {
        ...
        $body=Http::body();$answer=self::studentAnswer((string)$question['question_type'],$body);$expected=json_decode((string)$question['correct_answer_json'],true,flags:JSON_THROW_ON_ERROR);$correct=self::isCorrect((string)$question['question_type'],$answer,$expected);
        $attemptId=Uuid::v4();
        $db->beginTransaction();
        try {
    ...
    {
        $options=is_array($rawOptions)?array_map(static fn(mixed $v):string=>trim((string)$v),array_values($rawOptions)):[];
        if(in_array($type,['single_choice','multiple_choice'],true)){
            if(count($options)<2||count($options)>6||in_array('',$options,true))Http::error('validation_error','Добавьте от 2 до 6 заполненных вариантов',422);
            $correct=array_values(array_unique(array_map('intval',(array)($body['correctOptions']??(isset($body['correctOption'])?[$body['correctOption']]:[])))));
            if($correct===[]||min($correct)<0||max($correct)>=count($options)||($type==='single_choice'&&count($correct)!==1))Http::error('validation_error','Проверьте правильные варианты',422);
            sort($correct);return[$options,['options'=>$correct]];
        }
        if($type==='number'){$value=filter_var($body['correctNumber']??null,FILTER_VALIDATE_FLOAT);$tolerance=filter_var($body['tolerance']??0,FILTER_VALIDATE_FLOAT);if($value===false||$tolerance===false||(float)$tolerance<0)Http::error('validation_error','Укажите число и неотрицательную погрешность',422);return[[],['value'=>(float)$value,'tolerance'=>(float)$tolerance]];}
        $accepted=array_values(array_filter(array_map(static fn(mixed $v):string=>trim((string)$v),(array)($body['acceptedAnswers']??[])),static fn(string $v):bool=>$v!==''));if($accepted===[]||count($accepted)>10)Http::error('validation_error','Добавьте от 1 до 10 допустимых ответов',422);return[[],['accepted'=>$accepted]];
    }

    /** @return array<string,mixed> */
    private static function studentAnswer(string $type,array $body): array
    {
        if($type==='single_choice'){$value=filter_var($body['selectedOption']??null,FILTER_VALIDATE_INT);if($value===false||$value<0)Http::error('validation_error','Выберите вариант ответа',422);return['options'=>[(int)$value]];}
        if($type==='multiple_choice'){$values=array_values(array_unique(array_map('intval',(array)($body['selectedOptions']??[]))));if($values===[]||min($values)<0)Http::error('validation_error','Выберите хотя бы один вариант',422);sort($values);return['options'=>$values];}
        if($type==='number'){$value=filter_var($body['numberAnswer']??null,FILTER_VALIDATE_FLOAT);if($value===false)Http::error('validation_error','Введите числовой ответ',422);return['value'=>(float)$value];}
        $value=trim((string)($body['textAnswer']??''));if($value===''||mb_strlen($value)>500)Http::error('validation_error','Введите короткий ответ',422);return['value'=>$value];
    }

    private static function isCorrect(string $type,array $answer,array $expected): bool
    {
        if(in_array($type,['single_choice','multiple_choice'],true))return($answer['options']??[])===($expected['options']??[]);
        if($type==='number')return abs((float)$answer['value']-(float)$expected['value'])<=(float)($expected['tolerance']??0);
