...
final class PlanningApi
{
    public static function dispatch(PDO $db, string $method, string $path): never
    {
        ...
        match (true) {
            ...
            $method === 'GET' &&
                preg_match(
                    '#^/api/v1/students/([0-9a-f-]{36})/topics/([0-9a-f-]{36})/review-questions$#',
                    $path,
                    $m,
                ) === 1
        ...
        foreach ($rows as $index => $row) {
            $reason =
                $row['progress_status'] === 'in_progress'
                    ? 'продолжить начатый урок'
                    : ($row['mastery_status'] === 'needs_reinforcement'
                        ? 'тема требует закрепления'
            ...
            $items[] = [
                ...
                'scheduledDate' => $weekStart->modify('+' . min($index, 4) . ' days')->format('Y-m-d'),
                'isRequired' => $index < 3,
                'reason' => $reason,
            ];
        }
    ...
    private static function addItem(PDO $db, string $familyId, string $actorId, string $studentId): never
    {
        ...
        if (!$planId) {
            ...
            try {
                $db->prepare(
                    'INSERT INTO weekly_plans (id,family_id,student_id,week_start) VALUES (:id,:family_id,:student_id,:week_start)',
                )->execute([
                    'id' => $planId,
                    'family_id' => $familyId,
                    'student_id' => $studentId,
        ...
        try {
            $db->prepare(
                'INSERT INTO plan_items (id,family_id,weekly_plan_id,lesson_id,scheduled_date,is_required,position) VALUES (:id,:family_id,:plan_id,:lesson_id,:date,:required,:position)',
            )->execute([
                'id' => $id,
                'family_id' => $familyId,
                'plan_id' => $planId,
                'lesson_id' => $lessonId,
                'date' => $date,
                'required' => ($body['isRequired'] ?? true) === true ? 1 : 0,
                'position' => max(0, (int) ($body['position'] ?? 0)),
            ]);
        } catch (\PDOException $error) {
            if ($error->getCode() === '23000') {
                Http::error('duplicate_plan_item', 'Этот урок уже назначен на выбранный день', 409);
            }
            throw $error;
        }
    ...
    private static function progressReport(PDO $db, string $familyId, string $studentId): never
    {
        ...
        $items = array_map(
            static fn(array $row): array => [
                ...
                'planStatus' => self::planStatus($row),
                'startedAt' => $row['started_at'],
                'completedAt' => $row['completed_at'],
                'reflection' => $row['feeling']
                    ? ['feeling' => $row['feeling'], 'comment' => $row['comment'], 'updatedAt' => $row['reflected_at']]
                    : null,
            ],
            $statement->fetchAll(),
        );
        ...
        foreach ($items as $item) {
            $key = match ($item['progressStatus']) {
                'completed' => 'completed',
                'in_progress' => 'inProgress',
                default => 'notStarted',
            };
            $summary[$key]++;
            if (($item['reflection']['feeling'] ?? null) === 'need_help') {
                $summary['needsHelp']++;
            }
        }
        $mastery = Mastery::topics($db, $familyId, $studentId);
        $summary['masteredTopics'] = count(
            array_filter($mastery, static fn(array $topic): bool => $topic['status'] === 'mastered'),
        );
        $summary['topicsToReview'] = count(
            array_filter($mastery, static fn(array $topic): bool => $topic['status'] === 'needs_reinforcement'),
        );
        $achievements = Achievements::list($db, $familyId, $studentId);
        $summary['achievements'] = count($achievements);
        $reviewTasks = Mastery::reviewTasks($db, $familyId, $studentId);
        Http::json([
            'summary' => $summary,
            'dailyDigest' => self::dailyDigest($items),
            'aiDigest' => self::aiDigest($db, $familyId, $studentId),
            'weeklyDigest' => self::weeklyDigest($db, $familyId, $studentId, $items, $mastery, $reviewTasks),
            'items' => $items,
            'reviewTasks' => $reviewTasks,
            'masterySubjects' => Mastery::subjects($db, $familyId, $studentId),
            'mastery' => $mastery,
    ...
    private static function aiDigest(PDO $db, string $familyId, string $studentId): array
    {
        ...
        return [
            'date' => date('Y-m-d'),
            'hintCount' => count($rows),
            'lessonCount' => count(array_unique(array_column($rows, 'lesson_id'))),
            'items' => array_map(
                static fn(array $row): array => [
                    'id' => $row['id'],
                    'lessonId' => $row['lesson_id'],
            ...
            'conversationQuestion' => $latest
                ? 'Что сегодня было самым сложным в теме «' .
                    $latest['topic_title'] .
                    '» и какой шаг помог продвинуться?'
                : 'Что сегодня получилось лучше всего и что хочется разобрать вместе?',
        ];
    }
    ...
    {
        $today = date('Y-m-d');
        $todayItems = array_values(
            array_filter($items, static fn(array $item): bool => $item['scheduledDate'] === $today),
        );
        $counts = [
            'assigned' => 0,
            'inProgress' => 0,
        ...
        foreach ($todayItems as $item) {
            $key = match ($item['planStatus']) {
                'in_progress' => 'inProgress',
                'needs_revision' => 'needsRevision',
                default => $item['planStatus'],
            };
            $counts[$key]++;
            if ($item['progressStatus'] === 'completed') {
                $counts['completedLessons']++;
            }
            $reasons = [];
            if ($item['planStatus'] === 'needs_revision') {
                $reasons[] = 'работу нужно доработать';
            }
            if (($item['reflection']['feeling'] ?? null) === 'need_help') {
                $counts['needsHelp']++;
                $reasons[] = 'ребёнок попросил помощи';
            }
            if ($reasons !== []) {
                $attention[] = [
                    'id' => $item['id'],
        ...
        $planned = count($todayItems);
        $message =
            $planned === 0
                ? 'На сегодня заданий нет.'
                : ($attention !== []
                    ? 'Есть задания, которым нужно внимание.'
                    : ($counts['reviewed'] === $planned
                        ? 'План на сегодня полностью выполнен и проверен.'
                        : "Проверено {$counts['reviewed']} из {$planned} заданий на сегодня."));
    ...
    private static function weeklyDigest(
        PDO $db,
        string $familyId,
        string $studentId,
        array $items,
        ...
        $start = (new DateTimeImmutable('today'))->modify('monday this week');
        $end = $start->modify('+6 days');
        $startDate = $start->format('Y-m-d');
        $endDate = $end->format('Y-m-d');
        $weekItems = array_values(
            array_filter(
                $items,
                static fn(array $item): bool => $item['scheduledDate'] >= $startDate &&
                    $item['scheduledDate'] <= $endDate,
            ),
        );
        $reviewed = count(array_filter($weekItems, static fn(array $item): bool => $item['planStatus'] === 'reviewed'));
        $completedLessons = count(
            array_filter($weekItems, static fn(array $item): bool => $item['progressStatus'] === 'completed'),
        );

        $masteredStatement = $db->prepare(
        ...
        $masteredStatement->execute([
            'family_id' => $familyId,
            'student_id' => $studentId,
            'week_start' => $startDate,
            'week_end' => $endDate,
        ]);
        $masteredTopics = array_map(
            static fn(array $row): array => [
                'id' => $row['id'],
                'title' => $row['title'],
                'subjectTitle' => $row['subject_title'],
            ],
            $masteredStatement->fetchAll(),
        );
        ...
        foreach ($weekItems as $item) {
            $reasons = [];
            if ($item['planStatus'] === 'needs_revision') {
                $reasons[] = 'нужна доработка';
            }
            if (($item['reflection']['feeling'] ?? null) === 'need_help') {
                $reasons[] = 'ребёнок попросил помощи';
            }
            if ($reasons !== []) {
                $difficulties[$item['lessonId']] = [
                    'id' => $item['lessonId'],
                    'title' => $item['title'],
                    'subjectTitle' => $item['subjectTitle'],
                    'reason' => implode(' · ', $reasons),
                ];
                $suggestions['lesson-' . $item['lessonId']] = [
                    'kind' => 'lesson',
                    'title' => 'Вернуться к уроку «' . $item['title'] . '»',
                    'reason' => implode(' · ', $reasons),
                ];
            } elseif ($item['isRequired'] && in_array($item['planStatus'], ['assigned', 'in_progress'], true)) {
                $suggestions['lesson-' . $item['lessonId']] = [
                    'kind' => 'lesson',
                    'title' => 'Завершить «' . $item['title'] . '»',
                    'reason' => 'обязательное задание текущей недели ещё не завершено',
                ];
            }
        }
        foreach ($mastery as $topic) {
            if ($topic['status'] === 'needs_reinforcement') {
                $difficulties['topic-' . $topic['id']] = [
                    'id' => $topic['id'],
                    'title' => $topic['title'],
                    'subjectTitle' => $topic['subjectTitle'],
                    'reason' => 'тема требует закрепления',
                ];
            }
        }
        foreach ($reviewTasks as $task) {
            $suggestions['review-' . $task['topicId']] = [
                'kind' => 'review',
                'title' => 'Повторить тему «' . $task['topicTitle'] . '»',
                'reason' => $task['isDue'] ? 'повторение уже доступно' : 'повторение назначено на ' . $task['dueDate'],
            ];
        }
        foreach ($mastery as $topic) {
            if ($topic['status'] === 'needs_reinforcement' && !isset($suggestions['review-' . $topic['id']])) {
                $suggestions['topic-' . $topic['id']] = [
                    'kind' => 'topic',
                    'title' => 'Добавить практику по теме «' . $topic['title'] . '»',
                    'reason' => 'по теме недостаточно устойчивых подтверждений',
                ];
            }
        }
        $planned = count($weekItems);
        $percent = $planned === 0 ? 0 : (int) round(($reviewed / $planned) * 100);
        return [
            'weekStart' => $startDate,
            'weekEnd' => $endDate,
            'planned' => $planned,
    ...
    private static function removeItem(PDO $db, string $familyId, string $actorId, string $id): never
    {
        ...
        if ($statement->rowCount() === 0) {
            Http::error('not_found', 'Пункт плана не найден', 404);
        }
    ...
    private static function reviewQuestions(PDO $db, string $familyId, string $studentId, string $topicId): never
    {
        ...
        $explicit = array_values(
            array_filter($rows, static fn(array $row): bool => $row['selected_position'] !== null),
        );
        usort(
            $explicit,
            static fn(array $a, array $b): int => (int) $a['selected_position'] <=> (int) $b['selected_position'],
        );
        $selectedIds = $explicit === [] ? array_column(array_slice($rows, 0, 3), 'id') : array_column($explicit, 'id');
        Http::json([
            'mode' => $explicit === [] ? 'automatic' : 'custom',
    ...
    private static function saveReviewQuestions(
        PDO $db,
        string $familyId,
        string $actorId,
        string $studentId,
        ...
        if (!is_array($raw)) {
            Http::error('validation_error', 'Выберите от одного до трёх вопросов', 422);
        }
        $questionIds = array_values(array_unique(array_map(static fn(mixed $id): string => (string) $id, $raw)));
        if (count($questionIds) < 1 || count($questionIds) > 3 || count($questionIds) !== count($raw)) {
            Http::error('validation_error', 'Выберите от одного до трёх разных вопросов', 422);
        }
        foreach ($questionIds as $id) {
            if (preg_match('/^[0-9a-f-]{36}$/', $id) !== 1) {
                Http::error('validation_error', 'Некорректный вопрос', 422);
            }
        }
        $placeholders = implode(',', array_fill(0, count($questionIds), '?'));
        $check = $db->prepare(
            "SELECT q.id FROM quiz_questions q JOIN activities a ON a.id=q.activity_id JOIN lessons l ON l.id=a.lesson_id WHERE q.id IN ($placeholders) AND q.family_id=? AND l.topic_id=? AND a.deleted_at IS NULL AND l.deleted_at IS NULL",
        );
        $check->execute([...$questionIds, $familyId, $topicId]);
        if (count($check->fetchAll()) !== count($questionIds)) {
            Http::error('validation_error', 'Вопрос не относится к выбранной теме', 422);
        }
        $db->beginTransaction();
        try {
            $db->prepare(
                'DELETE FROM review_question_settings WHERE family_id=:family_id AND student_id=:student_id AND topic_id=:topic_id',
            )->execute(['family_id' => $familyId, 'student_id' => $studentId, 'topic_id' => $topicId]);
            $insert = $db->prepare(
                'INSERT INTO review_question_settings(id,family_id,student_id,topic_id,question_id,position) VALUES(:id,:family_id,:student_id,:topic_id,:question_id,:position)',
            );
            ...
            Audit::record($db, $familyId, 'parent', $actorId, 'review.questions_configured', 'topic', $topicId, [
    ...
    {
        $date = self::date((string) (Http::body()['dueDate'] ?? ''));
        $s = $db->prepare(
            'UPDATE review_schedule SET due_date=:due_date WHERE id=:id AND family_id=:family_id AND status=\'pending\'',
        );
        $s->execute(['due_date' => $date, 'id' => $id, 'family_id' => $familyId]);
        if ($s->rowCount() === 0) {
            Http::error('not_found', 'Активное повторение не найдено', 404);
        }
        $db->prepare(
            'UPDATE mastery_states ms JOIN review_schedule rs ON rs.student_id=ms.student_id AND rs.topic_id=ms.topic_id SET ms.next_review_at=:due_date WHERE rs.id=:id',
        )->execute(['due_date' => $date, 'id' => $id]);
        Audit::record($db, $familyId, 'parent', $actorId, 'review.rescheduled', 'review_schedule', $id, [
            'dueDate' => $date,
        ]);
        Http::json(['reviewTask' => ['id' => $id, 'dueDate' => $date]]);
    }

    private static function assertStudent(PDO $db, string $familyId, string $studentId): void
    {
        $statement = $db->prepare(
            'SELECT id FROM students WHERE id=:id AND family_id=:family_id AND deleted_at IS NULL',
        );
    ...
    {
        $statement = $db->prepare(
            'SELECT t.id FROM curricula c JOIN curriculum_subjects cs ON cs.curriculum_id=c.id JOIN sections se ON se.curriculum_subject_id=cs.id JOIN topics t ON t.section_id=se.id WHERE c.student_id=:student_id AND c.family_id=:family_id AND t.id=:topic_id AND c.deleted_at IS NULL AND t.deleted_at IS NULL',
        );
        $statement->execute(['student_id' => $studentId, 'family_id' => $familyId, 'topic_id' => $topicId]);
        if (!$statement->fetchColumn()) {
            Http::error('not_found', 'Тема не найдена в программе ученика', 404);
        }
    }
    ...
    private static function date(string $value): string
    {
        ...
        if (!$date || $date->format('Y-m-d') !== $value) {
            Http::error('validation_error', 'Укажите корректную дату', 422);
        }
        return $value;
    }
    ...
    private static function lesson(array $row): array
    {
        return [
            'id' => $row['id'],
            'title' => $row['title'],
    ...
    {
        if ((int) ($row['revision_count'] ?? 0) > 0) {
            return 'needs_revision';
        }
        if ((int) ($row['submitted_count'] ?? 0) > 0) {
            return 'submitted';
        }
        $homeworkCount = (int) ($row['homework_count'] ?? 0);
        ...
        if (
            ...
            (int) ($row['draft_count'] ?? 0) > 0 ||
            (int) ($row['reviewed_count'] ?? 0) > 0 ||
            (int) ($row['attempted_quiz_count'] ?? 0) > 0
        ) {
            return 'in_progress';
        }
        return 'assigned';
