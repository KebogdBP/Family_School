...
function SetupPage() {
  ...
  return (
    <AuthLayout
      title="Создание семейного пространства"
      subtitle="Выполняется один раз после установки HomeEdu на сервер."
    >
      <form
        className="stack"
        onSubmit={(event) => {
      ...
      >
        ...
        <Field label="Название семьи">
          <input required value={form.familyName} onChange={(event) => update('familyName', event.target.value)} />
        </Field>
        ...
        <Field label="Пароль (минимум 12 символов)">
          <input
            type="password"
            minLength={12}
            required
  ...
  return (
    <main id="main-content" className="auth-page">
      <section className="auth-card">
        ...
        <p className="eyebrow">Семейное образование</p>
        <h1>{title}</h1>
  ...
  return (
    <div className="app-shell">
      <a
        className="skip-link"
        href="#main-content"
        onClick={() => setTimeout(() => document.getElementById('main-content')?.focus())}
      >
      ...
      <aside className="sidebar">
        ...
        <nav aria-label="Основная навигация">
          {principal.role === 'parent' ? (
            ...
            <>
              ...
              <NavLink to="/diagnostic">Диагностика</NavLink>
        ...
        <div className="account">
          ...
          <small>{principal.role === 'parent' ? 'Родитель' : `${principal.grade} класс`}</small>
          <button className="button-secondary" onClick={() => exit.mutate()}>
            Выйти
          </button>
        </div>
      </aside>
...
function CreateStudentForm({ onDone }: { onDone: () => void }) {
  ...
  return (
    ...
    >
      ...
      <Field label="PIN ребёнка (4–8 цифр)">
        <input
          inputMode="numeric"
          pattern="[0-9]{4,8}"
          required
...
function StudentCard({ student, onLoggedIn }: { student: Student; onLoggedIn: (principal: Principal) => void }) {
  ...
  return (
    <article className="card child-card">
      <div className="avatar" style={{ background: student.avatarColor }}>
        {student.displayName.slice(0, 1).toUpperCase()}
      </div>
      <div>
        <h2>{student.displayName}</h2>
        <p className="muted">
          {student.grade} класс{student.age ? ` · ${student.age} лет` : ''}
        </p>
      </div>
      <span className="badge">Профиль ученика</span>
      <div className="card-actions">
        <NavLink className="button-link button-ghost" to={`/children/${student.id}/curriculum`}>
          Учебная программа
        </NavLink>
        <NavLink className="button-link button-ghost" to={`/children/${student.id}/plan`}>
          План недели
        </NavLink>
        <NavLink className="button-link button-ghost" to={`/children/${student.id}/report`}>
          Отчёт
        </NavLink>
        {open ? (
          <form
          ...
          >
            <input
              aria-label={`PIN для ${student.displayName}`}
              autoFocus
              inputMode="numeric"
              pattern="[0-9]{4,8}"
            ...
            />
            <button disabled={mutation.isPending}>Войти</button>
            <ErrorText error={mutation.error} />
          </form>
        ) : (
          <button onClick={() => setOpen(true)}>Перейти в профиль</button>
        )}
        <button className="button-danger button-small" onClick={() => setDeleting(true)}>
          Удалить профиль
        </button>
      </div>
      {deleting && (
        <form
          className="delete-student-confirmation"
          onSubmit={(event) => {
            event.preventDefault()
            remove.mutate()
          }}
        >
          <h3>Удалить профиль без возможности восстановления?</h3>
          <p>
            Будут удалены программа, прогресс, ответы, оценки и приватные файлы. Сначала сделайте экспорт данных, если
            они нужны.
          </p>
          <Field label={`Введите имя «${student.displayName}»`}>
            <input
              required
              autoComplete="off"
              value={confirmation}
          ...
          <div className="form-row">
            <button type="button" className="button-ghost" onClick={() => setDeleting(false)}>
              Отмена
            </button>
            <button
              className="button-danger"
              disabled={confirmation !== student.displayName || !parentPassword || remove.isPending}
            >
              {remove.isPending ? 'Удаляем…' : `Удалить ${student.displayName}`}
            </button>
          </div>
        </form>
      )}
...
function ChildrenPage() {
  ...
  const download = (data: FamilyExport) => {
    const url = URL.createObjectURL(new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }))
    const link = document.createElement('a')
    link.href = url
    link.download = `homeedu-family-${data.generatedAt.slice(0, 10)}.json`
  ...
  const loggedIn = (principal: Principal) => {
    setPrincipal(principal)
    client.clear()
  ...
  return (
    <>
      <header className="page-header">
        <p className="eyebrow">Семейное пространство</p>
        <div className="header-row">
          <div>
            <h1>Ученики</h1>
            <p className="muted">Выберите профиль или добавьте нового ребёнка.</p>
          </div>
          <div className="header-actions">
            <button className="button-ghost" disabled={familyExport.isPending} onClick={() => familyExport.mutate()}>
              {familyExport.isPending ? 'Готовим экспорт…' : 'Скачать данные'}
            </button>
            <button onClick={() => setShowForm((value) => !value)}>
              {showForm ? 'Закрыть' : '+ Добавить ребёнка'}
            </button>
          </div>
        </div>
      </header>
      ...
      <ErrorText error={familyExport.error} />
      {students.error ? (
        <ErrorState error={students.error} onRetry={() => void students.refetch()} />
      ) : students.isLoading ? (
        <LoadingState label="Загружаем профили…" />
      ) : (
        <section className="children-grid">
          {students.data?.students.map((student) => (
            <StudentCard key={student.id} student={student} onLoggedIn={loggedIn} />
          ))}
          {students.data?.students.length === 0 && (
            <EmptyState
              title="Пока нет учеников"
              description="Добавьте первый профиль, укажите класс и отдельный PIN для входа ребёнка."
            >
              <button onClick={() => setShowForm(true)}>Добавить ребёнка</button>
            </EmptyState>
          )}
        </section>
      )}
  ...
  return (
    <>
      <header className="page-header">
        ...
        <h1>Привет, {principal.displayName}!</h1>
        <p className="muted">
          {principal.grade} класс · пройдено {completed} из {lessons.data?.lessons.length ?? 0}
        </p>
      </header>
      {lessons.error ? (
        <ErrorState error={lessons.error} onRetry={() => void lessons.refetch()} />
      ) : (
        <>
          {continuation && (
            <section className="continue-lesson card" style={{ borderLeftColor: continuation.subjectColor }}>
              <div>
                <span className="badge">Продолжить занятие</span>
                <h2>{continuation.title}</h2>
                <p className="muted">
                  {continuation.subjectTitle} · {continuation.topicTitle}
                </p>
                <div className="progress-track">
                  <div
                    style={{
                      width: `${continuation.blockCount ? Math.round((continuation.progress.lastBlockPosition / continuation.blockCount) * 100) : 0}%`,
                      background: continuation.subjectColor,
                    }}
                  />
                </div>
              </div>
              <NavLink className="button-link" to={`/study/lessons/${continuation.id}`}>
                Продолжить →
              </NavLink>
            </section>
          )}
          {reviews.length > 0 && (
            <section className="review-reminder card">
              <div>
                <span className="badge">На сегодня</span>
                <h2>Короткая контрольная</h2>
                <p className="muted">Система выбрала несколько вопросов по теме.</p>
              </div>
              <ul>
                {reviews.map((task) => (
                  <li key={task.id}>
                    <strong>{task.topicTitle}</strong>
                    <span>
                      {task.subjectTitle} · {task.reason}
                    </span>
                    <NavLink className="button-link button-small" to={`/reviews/${task.id}`}>
                      Начать повторение →
                    </NavLink>
                  </li>
                ))}
          ...
          {lessons.isLoading ? (
            <LoadingState label="Собираем план на сегодня…" />
          ) : lessons.data?.lessons.length ? (
            <section className="student-lessons">
              {lessons.data.lessons.map((lesson) => (
                <StudentLessonCard key={lesson.planItemId} lesson={lesson} />
              ))}
            </section>
          ) : reviews.length === 0 ? (
            <EmptyState
              title="На сегодня всё свободно"
              description="В плане нет уроков и повторений. Можно отдохнуть или выбрать дополнительную практику вместе с родителем."
            />
          ) : null}
        </>
      )}
...
function StudentLessonCard({ lesson }: { lesson: TodayLesson }) {
  ...
  const percent =
    lesson.progress.status === 'completed'
      ? 100
      : lesson.blockCount
        ? Math.round((lesson.progress.lastBlockPosition / lesson.blockCount) * 100)
        : 0
  return (
    <article className="card student-lesson-card" style={{ borderTopColor: lesson.subjectColor }}>
      <div className="lesson-card-heading">
        <span className="badge" style={{ color: lesson.subjectColor }}>
          {lesson.subjectTitle}
        </span>
        <span className={`lesson-status status-${lesson.planStatus}`}>{statusLabels[lesson.planStatus]}</span>
      </div>
      <h2>{lesson.title}</h2>
      <p className="muted">
      ...
      <div className="progress-track">
        <div style={{ width: `${percent}%`, background: lesson.subjectColor }} />
      </div>
      <div className="lesson-card-footer">
        <small>
          {lesson.blockCount} шагов{lesson.estimatedMinutes ? ` · ${lesson.estimatedMinutes} мин` : ''}
        </small>
        <NavLink className="button-link" to={`/study/lessons/${lesson.id}`}>
          {labels[lesson.progress.status]} →
        </NavLink>
      </div>
    </article>
  )
...
function RoutedApp() {
  ...
  useEffect(() => {
    if (session.data) setPrincipal(session.data.principal)
    if (session.error instanceof ApiError && session.error.status === 401) setPrincipal(null)
  }, [session.data, session.error, setPrincipal])
  ...
  if (session.error && !(session.error instanceof ApiError && session.error.status === 401))
    return (
      <main>
        <ErrorState fullPage error={session.error} onRetry={() => void session.refetch()} />
      </main>
    )
  return (
    <Routes>
      <Route
        path="/login"
        element={
          principal ? <Navigate to={principal.role === 'parent' ? '/children' : '/today'} replace /> : <LoginPage />
        }
      />
      ...
      />
      <Route
        path="/children/:studentId/curriculum"
        element={
          principal?.role === 'parent' ? (
            <AppShell principal={principal}>
              <CurriculumPage />
      ...
      />
      <Route
        path="/children/:studentId/report"
        element={
          principal?.role === 'parent' ? (
            <AppShell principal={principal}>
              <ProgressReportPage />
      ...
      <Route
        ...
        element={
          principal?.role === 'student' ? (
            <AppShell principal={principal}>
              <TodayPage principal={principal} />
            </AppShell>
          ) : (
            <Navigate to={principal ? '/children' : '/login'} replace />
          )
        }
      />
      <Route
      ...
      <Route
        ...
        element={
          principal ? (
            <AppShell principal={principal}>
              <NotFoundState home={principal.role === 'student' ? '/today' : '/children'} />
            </AppShell>
          ) : (
            <NotFoundState home="/login" />
          )
        }
      />
    </Routes>
