...
import {
  approveAiQuizDraft, createLessonBlock, deleteLessonBlock, generateAiQuizDraft, getAiQuizDrafts, getLessonContent, updateLessonBlock,
...
function AiQuizDrafts({ lessonId }: { lessonId: string }) {
  const client=useQueryClient();const drafts=useQuery({queryKey:['ai-quiz-drafts',lessonId],queryFn:()=>getAiQuizDrafts(lessonId)});const refresh=async()=>{await Promise.all([client.invalidateQueries({queryKey:['ai-quiz-drafts',lessonId]}),client.invalidateQueries({queryKey:['lesson-quizzes',lessonId]})])}
  const generate=useMutation({mutationFn:()=>generateAiQuizDraft(lessonId),onSuccess:refresh});const approve=useMutation({mutationFn:approveAiQuizDraft,onSuccess:refresh})
  return <section className="card ai-drafts"><div className="header-row"><div><span className="badge">AI · только черновик</span><h2>Черновик теста</h2><p className="muted">Проверь вопрос и правильный ответ. Ребёнок увидит тест только после утверждения.</p></div><button disabled={generate.isPending} onClick={()=>generate.mutate()}>{generate.isPending?'Готовим…':'Создать черновик'}</button></div>{drafts.data?.drafts.map((draft)=><article className="ai-draft" key={draft.id}><strong>{draft.title}</strong><p>{draft.prompt}</p><ol>{draft.options.map((option,index)=><li className={index===draft.correctOption?'correct-draft-option':''} key={option}>{option}</li>)}</ol><p className="muted">Пояснение: {draft.explanation}</p>{draft.status==='draft'?<button className="button-small" disabled={approve.isPending} onClick={()=>approve.mutate(draft.id)}>Утвердить и добавить тест</button>:<span className="saved-note">Утверждён родителем</span>}</article>)}<ErrorText error={drafts.error??generate.error??approve.error}/></section>
}
