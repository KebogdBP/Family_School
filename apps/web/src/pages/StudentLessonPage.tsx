...
function HomeworkCard({ homework, refresh }: { homework: StudentHomework; refresh: () => Promise<unknown> }) {
  ...
  return <form className="card student-homework" onSubmit={(event) => { event.preventDefault(); save.mutate(true) }}>
    ...
    <div aria-live="polite">{homework.submission?.status === 'submitted' && <p className="saved-note">Работа отправлена и ждёт проверки.</p>}{homework.submission?.status === 'reviewed' && <p className="saved-note">Работа принята.</p>}</div>{error && <p className="form-error" role="alert">{error.message}</p>}
  </form>
}
...
export function StudentLessonPage() {
  ...
  return <div className="focus-lesson"><header className="lesson-focus-header"><NavLink className="text-link" to="/today">← Мои уроки</NavLink><div><p className="eyebrow" style={{ color: lesson.subjectColor }}>{lesson.subjectTitle}</p><h1>{lesson.title}</h1><p className="muted">{lesson.sectionTitle} · {lesson.topicTitle}</p></div><div className="progress-wrap"><div className="progress-label"><strong>{completed ? 'Готово!' : 'Прохождение урока'}</strong><span>{completed ? 100 : progress}%</span></div><div className="progress-track"><div style={{ width: `${completed ? 100 : progress}%`, background: lesson.subjectColor }} /></div></div></header>
    ...
    {lesson.blocks.length > 0 && <footer className="lesson-next">{completed ? <><div className="completion-card"><span aria-hidden="true">✓</span><div><h2>Урок пройден</h2><p>Отличная работа! Результат сохранён.</p></div><NavLink className="button-link" to="/today">К списку уроков</NavLink></div><form className="card reflection-card" onSubmit={(event) => { event.preventDefault(); reflection.mutate() }}><div><h2>Как тебе было?</h2><p className="muted">Это не оценка — ответ поможет подобрать следующий урок.</p></div><div className="feeling-options" role="group" aria-label="Самооценка урока">{([['easy','Легко'],['good','Получилось'],['hard','Трудно'],['need_help','Нужна помощь']] as const).map(([value,label]) => <button type="button" aria-pressed={feeling === value} className={feeling === value ? 'feeling-selected' : 'button-ghost'} key={value} onClick={() => setFeeling(value)}>{label}</button>)}</div><label className="field"><span>Комментарий к самооценке (необязательно)</span><textarea maxLength={500} rows={2} placeholder="Можно коротко написать, что было сложно" value={comment} onChange={(event) => setComment(event.target.value)} /></label><button disabled={!feeling || reflection.isPending}>{lesson.reflection || reflection.isSuccess ? 'Обновить ответ' : 'Сохранить ответ'}</button><div aria-live="polite">{lesson.reflection && !reflection.isSuccess && <p className="saved-note">Ответ уже сохранён: {lesson.reflection.feeling === 'need_help' ? 'нужна помощь' : 'спасибо!'}</p>}{reflection.isSuccess && <p className="saved-note">Спасибо, ответ сохранён.</p>}</div>{reflection.error && <p className="form-error" role="alert">{reflection.error.message}</p>}</form></> : <button disabled={save.isPending} onClick={next}>{visibleCount + 1 >= lesson.blocks.length ? 'Завершить урок' : 'Дальше →'}</button>}{save.error && <p className="form-error" role="alert">{save.error.message}</p>}</footer>}
  </div>
}
