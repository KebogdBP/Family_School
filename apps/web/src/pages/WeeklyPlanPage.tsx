...
import { addPlanItem, deletePlanItem, getPlanDraft, getWeeklyPlan, type PlanDraftItem, type PlanItem } from '@/entities/planning/api'
...
function DraftEditor({ studentId, week, refresh }: { studentId: string; week: string; refresh: () => Promise<unknown> }) {
  const [items,setItems]=useState<PlanDraftItem[]|null>(null)
  const generate=useMutation({mutationFn:()=>getPlanDraft(studentId,week),onSuccess:(draft)=>setItems(draft.items)})
  const apply=useMutation({mutationFn:async()=>{for(const [position,item] of (items??[]).entries())await addPlanItem(studentId,{lessonId:item.lessonId,scheduledDate:item.scheduledDate,isRequired:item.isRequired,position})},onSuccess:async()=>{setItems(null);await refresh()}})
  if(items===null)return <section className="card plan-draft-intro"><div><span className="badge">Следующая неделя</span><h2>Черновик по прогрессу</h2><p className="muted">Незавершённые и требующие закрепления уроки попадут в начало. Ничего не назначается без подтверждения.</p></div><button disabled={generate.isPending} onClick={()=>generate.mutate()}>{generate.isPending?'Собираем…':'Собрать черновик'}</button>{generate.error&&<p className="form-error">{generate.error.message}</p>}</section>
  return <section className="card plan-draft"><div className="header-row"><div><span className="badge">Черновик · можно изменить</span><h2>Предложено уроков: {items.length}</h2></div><button className="button-ghost" onClick={()=>setItems(null)}>Отменить</button></div>{items.length?<div className="plan-draft-list">{items.map((item,index)=><article key={item.lessonId}><div><strong>{item.title}</strong><small>{item.subjectTitle} · {item.topicTitle} · {item.reason}</small></div><input aria-label={`Дата: ${item.title}`} type="date" min={week} max={shiftWeek(week,6)} value={item.scheduledDate} onChange={(event)=>setItems(items.map((value,i)=>i===index?{...value,scheduledDate:event.target.value}:value))}/><label className="check-field"><input type="checkbox" checked={item.isRequired} onChange={(event)=>setItems(items.map((value,i)=>i===index?{...value,isRequired:event.target.checked}:value))}/><span>Обязательно</span></label><button className="icon-button icon-danger" aria-label={`Убрать ${item.title}`} onClick={()=>setItems(items.filter((_,i)=>i!==index))}>×</button></article>)}</div>:<p className="muted">Подходящих незавершённых уроков нет.</p>}<button disabled={!items.length||apply.isPending} onClick={()=>apply.mutate()}>{apply.isPending?'Добавляем…':'Утвердить и добавить в план'}</button>{apply.error&&<p className="form-error">{apply.error.message}</p>}</section>
}
