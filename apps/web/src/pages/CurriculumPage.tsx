...
import {
  ...
  deleteCurriculumNode, getCurricula, getCurriculum, getSubjects, installDavidFractions, installSaraFractions, updateCurriculumNode, updateMasterySettings,
...
function MasterySettingsForm({ subject, onChanged }: { subject: AssignedSubject; onChanged: () => Promise<unknown> }) {
  const [open, setOpen] = useState(false)
  const [form, setForm] = useState(subject.masterySettings)
  const save = useMutation({ mutationFn: () => updateMasterySettings(subject.assignmentId, form), onSuccess: async () => { setOpen(false); await onChanged() } })
  if (!open) return <button className="button-ghost button-small" onClick={() => setOpen(true)}>Правила освоения</button>
  return <form className="mastery-settings" onSubmit={(event) => { event.preventDefault(); save.mutate() }}><label className="field"><span>Всего подтверждений</span><input type="number" min="1" max="10" value={form.minEvidenceCount} onChange={(event) => setForm({ ...form, minEvidenceCount: Number(event.target.value) })} /></label><label className="field"><span>Разных типов работы</span><input type="number" min="1" max="3" value={form.minSuccessfulTypes} onChange={(event) => setForm({ ...form, minSuccessfulTypes: Number(event.target.value) })} /></label><label className="field"><span>Повторить через, дней</span><input type="number" min="1" max="60" value={form.reviewIntervalDays} onChange={(event) => setForm({ ...form, reviewIntervalDays: Number(event.target.value) })} /></label><div><button disabled={save.isPending}>Сохранить</button><button type="button" className="button-ghost" onClick={() => setOpen(false)}>Отмена</button></div><ErrorText error={save.error} /></form>
}
...
function PilotRouteInstaller({ studentId, grade }: { studentId: string; grade: number }) {
  ...
  const seed = useMutation({ mutationFn: () => install(studentId), onSuccess: async (result) => { await Promise.all([client.invalidateQueries({ queryKey: ['curricula', studentId] }), client.invalidateQueries({ queryKey: ['curriculum', result.curriculumId] }), client.invalidateQueries({ queryKey: ['subjects'] })]) } })
  return <div className="pilot-route-action"><button disabled={seed.isPending || Boolean(seed.data)} onClick={() => seed.mutate()}>{seed.isPending ? 'Устанавливаем…' : seed.data ? 'Маршрут установлен' : `Добавить маршрут «${grade === 4 ? 'Дроби' : 'Обыкновенные дроби'}»`}</button><ErrorText error={seed.error} />{seed.data?.installed && <small className="seed-inline-status" role="status">Готово: {seed.data.counts?.topics} темы, {seed.data.counts?.lessons} уроков, {seed.data.counts?.competencies} навыков</small>}</div>
}
...
export function CurriculumPage() {
  ...
  return <><header className="page-header"><p className="eyebrow">Учебная программа</p><div className="header-row"><div><h1>{student ? `Программа: ${student.displayName}` : 'Программа ученика'}</h1><p className="muted">Предметы, разделы, темы и уроки в правильном порядке.</p></div><div className="header-actions">{student && [4,6].includes(student.grade) && <PilotRouteInstaller key={student.id} studentId={student.id} grade={student.grade} />}<NavLink className="button-link button-ghost" to="/children">← К ученикам</NavLink></div></div></header><ErrorText error={students.error ?? curricula.error ?? tree.error} />{students.isLoading || curricula.isLoading ? <p>Загружаем программу…</p> : !student ? <div className="card empty"><h2>Ученик не найден</h2></div> : !curriculumId ? <CurriculumCreator studentId={student.id} grade={student.grade} /> : tree.data ? <><SubjectToolbar tree={tree.data.curriculum} /><ProgramTree tree={tree.data.curriculum} /></> : <p>Загружаем содержание…</p>}</>
}
