...
export type StudentQuiz = {
  id: string
  title: string
  question: { id: string; prompt: string; questionType: QuestionType; options: string[] }
}
...
export type StudentHomework = {
  ...
  submission: null | {
    id: string
    responseText: string
    status: 'draft' | 'submitted' | 'needs_revision' | 'reviewed'
...
export type MasteryTopic = {
  ...
  subjectTitle: string
  subjectColor: string
  status: 'available' | 'learning' | 'needs_reinforcement' | 'mastered'
  score: number
...
export type Achievement = {
  id: string
  code: 'independent_revision' | 'independent_explanation' | 'durable_mastery'
  title: string
  description: string
  earnedAt: string
}
...
export const getTodayLessons = () =>
  api<{ date: string; lessons: TodayLesson[]; continueLesson: ContinueLesson | null; reviewTasks: ReviewTask[] }>(
    '/student/today',
  )
...
export const submitDiagnostic = (answers: number[]) =>
  api<{ completed: boolean; result: DiagnosticResult }>('/student/diagnostic', {
    method: 'POST',
    body: JSON.stringify({ answers }),
  })
export const getStudentReviewTasks = () => api<{ reviewTasks: ReviewTask[] }>('/student/reviews')
export const getReviewSession = (id: string) =>
  api<{
    review: {
      id: string
      topicTitle: string
...
export const submitReviewSession = (id: string, answers: Array<QuizAnswer & { questionId: string }>) =>
  api<{
    attempt: {
      id: string
      score: number
      successful: boolean
      results: Array<{ questionId: string; correct: boolean; explanation: string | null }>
    }
  }>(`/student/reviews/${id}`, { method: 'POST', body: JSON.stringify({ answers }) })
...
export const saveLessonProgress = (lessonId: string, lastBlockPosition: number, completed = false) =>
  api<{ progress: Progress }>(`/student/lessons/${lessonId}/progress`, {
    method: 'PATCH',
    body: JSON.stringify({ lastBlockPosition, completed }),
  })
export const saveReflection = (lessonId: string, feeling: ReflectionFeeling, comment: string) =>
  api<{ reflection: Reflection }>(`/student/lessons/${lessonId}/reflection`, {
    method: 'POST',
    body: JSON.stringify({ feeling, comment }),
  })
...
export const submitQuizAttempt = (quizId: string, answer: QuizAnswer) =>
  api<{ attempt: { id: string; correct: boolean; score: number; explanation: string | null } }>(
    `/student/quizzes/${quizId}/attempts`,
    { method: 'POST', body: JSON.stringify(answer) },
  )
export const saveHomeworkSubmission = (homeworkId: string, responseText: string, submit: boolean) =>
  api<{ submission: StudentHomework['submission'] }>(`/student/homeworks/${homeworkId}/submission`, {
    method: 'PUT',
    body: JSON.stringify({ responseText, submit }),
  })
export const uploadHomeworkFile = (homeworkId: string, file: File) => {
  const body = new FormData()
  body.set('file', file)
  return api<{ file: SubmissionFile }>(`/student/homeworks/${homeworkId}/files`, { method: 'POST', body })
}
export const deleteHomeworkFile = (fileId: string) =>
  api<{ status: string }>(`/submission-files/${fileId}`, { method: 'DELETE' })
export const requestAiHint = (lessonId: string, question: string) =>
  api<{ hint: string; provider: string; safety: 'no_direct_answer' }>(`/student/lessons/${lessonId}/ai-hints`, {
    method: 'POST',
