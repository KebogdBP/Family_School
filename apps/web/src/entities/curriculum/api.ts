...
export type LessonBlock = {
  id: string
  blockType: BlockType
  content: { text?: string; url?: string; caption?: string }
  position: number
}
...
export type ParentQuiz = {
  ...
  question: {
    ...
    options: string[]
    correctAnswer: { options?: number[]; value?: number; tolerance?: number; accepted?: string[] }
...
export const createCurriculum = (input: { studentId: string; title: string; schoolYear: string }) =>
  api<{ curriculum: CurriculumSummary & { studentId: string } }>('/curricula', {
    method: 'POST',
...
export const attachSubject = (curriculumId: string, subjectId: string, position: number) =>
  api<{ curriculumSubject: { id: string } }>(`/curricula/${curriculumId}/subjects`, {
    method: 'POST',
    body: JSON.stringify({ subjectId, position }),
  })
export const updateMasterySettings = (assignmentId: string, settings: MasterySettings) =>
  api<{ settings: MasterySettings }>(`/curriculum-subjects/${assignmentId}/mastery-settings`, {
    method: 'PATCH',
...
export const getLessonContent = (lessonId: string) => api<{ lesson: LessonContent }>(`/lessons/${lessonId}/content`)

export const createLessonBlock = (
  lessonId: string,
  input: { blockType: BlockType; content: LessonBlock['content']; position: number },
...
export const createQuiz = (lessonId: string, input: CreateQuizInput) =>
  api<{ quiz: { id: string } }>(`/lessons/${lessonId}/quizzes`, { method: 'POST', body: JSON.stringify(input) })
...
export const createHomework = (lessonId: string, input: { title: string; instructions: string; position: number }) =>
  api<{ homework: { id: string } }>(`/lessons/${lessonId}/homeworks`, { method: 'POST', body: JSON.stringify(input) })
export const deleteHomework = (id: string) => api<{ status: string }>(`/homeworks/${id}`, { method: 'DELETE' })
export type PilotContentResult = {
  installed: boolean
...
export const installSaraFractions = (studentId: string) =>
  api<PilotContentResult>(`/students/${studentId}/pilot-content/sara-fractions`, { method: 'POST' })
...
export const getAiQuizDrafts = (lessonId: string) =>
  api<{ drafts: AiQuizDraft[] }>(`/lessons/${lessonId}/ai-quiz-drafts`)
export const generateAiQuizDraft = (lessonId: string) =>
  api<{ draft: AiQuizDraft }>(`/lessons/${lessonId}/ai-quiz-drafts`, { method: 'POST' })
export const approveAiQuizDraft = (draftId: string) =>
  api<{ approved: boolean; activityId: string }>(`/ai-quiz-drafts/${draftId}/approve`, { method: 'POST' })
